...
class NetfluxSupabaseUser extends BaseAuthUser {
  NetfluxSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> netfluxSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = NetfluxSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
